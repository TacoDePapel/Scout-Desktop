...
// opts.screenshotFn: async () => base64 JPEG of the primary screen, or null.
// When provided, the recorder snapshots the screen at start, at every click
// (throttled), and at stop — so a replay agent can SEE what each click hit
// instead of guessing from bare coordinates.
function startRecording(opts = {}) {
  ...
  const events = []
  const shots = []
  const rawShotFn = typeof opts.screenshotFn === 'function' ? opts.screenshotFn : null
  // A screenshot must NEVER block recording — race every call against a
  // timeout so a hung capture (Wayland portal, etc.) can't freeze start/stop.
  // Screen capture is slow on some compositors (~4s on GNOME/Wayland), so the
  // guard has to exceed that or every shot comes back null — but it still caps
  // the wait so a truly hung capture can't freeze recording.
  const screenshotFn = rawShotFn
    ? () => Promise.race([
        rawShotFn().catch(() => null),
  ...
  const takeShot = () => {
    if (!screenshotFn || shotInFlight) return     // don't pile up slow captures
    const now = Date.now()
    if (now - lastShotAt < 800) return            // throttle rapid clicking
    lastShotAt = now
    const t = now - startedAt
    shotInFlight = true
    screenshotFn()
      .then(b64 => { if (b64 && recording) shots.push({ t, jpg: b64 }) })
      .catch(() => {})
      .finally(() => { shotInFlight = false })
  }
  takeShot()                                      // initial screen state

  const handlers = {
    keydown: (e) => events.push({ k: 'kd', t: Date.now() - startedAt, key: e.keycode }),
    keyup:   (e) => events.push({ k: 'ku', t: Date.now() - startedAt, key: e.keycode }),
    mousedown: (e) => { takeShot(); events.push({ k: 'md', t: Date.now() - startedAt, x: e.x, y: e.y, b: e.button }) },
  ...
  try { uIOhook.start() }
    ...
    // libuiohook's AXAPI error ("Failed to enable access for assistive
    // devices") means macOS Accessibility permission is missing — translate
    // it, since the raw message points nowhere.
    if (process.platform === 'darwin' && /assistive|accessibility|axapi/i.test(e.message)) {
      return {
        error: 'macOS blocked the input hook. Enable Scout in System Settings → Privacy & Security → Accessibility (and Input Monitoring, if listed), then try again — quit and reopen Scout if it still fails.',
  ...
  recording = { startedAt, events, handlers, shots, screenshotFn }
...
async function stopRecording(name) {
  ...
  // Final screenshot — the outcome state, evidence for "done when".
  if (recording.screenshotFn) {
    const t = Date.now() - recording.startedAt
    try {
      const b64 = await recording.screenshotFn()
      if (b64) recording.shots.push({ t, jpg: b64 })
    } catch {}
  }
    ...
    switch (ev.k) {
      ...
      case 'md': {
        ...
        if (lastClick && lastClick.b === ev.b && ev.t - lastClick.t < 500 &&
          ...
          // Reference the nearest screenshot to this click. Capture can lag the
          // click by a few seconds, so match to the closest shot at/after it.
          let si = -1, best = Infinity
          shots.forEach((s, i) => { const d = Math.abs(s.t - ev.t); if (d < best && d < 6000) { best = d; si = i } })
          const ref = si >= 0 ? ` — see screenshot ${si + 1}` : ''
          lines.push(`[${fmtOffset(ev.t)}] ${btn} at (${ev.x}, ${ev.y})${ref}`)
